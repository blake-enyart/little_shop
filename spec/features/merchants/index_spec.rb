    ...
    describe "shows merchant leaderboard statistics" do
      ...
      describe 'top 5 in state/city' do
        before(:each) do
          ...
          @user_1 = create(:user, city: 'Denver', state: 'CO') #user for viewing merchants page
          user_2 = create(:user, city: 'Seattle', state: 'WA')
          user_3 = create(:user, city: 'Denver', state: 'CO')

          #generates hash of 10 new packaged orders to Denver, CO
          ...
          #generates hash of 5 new packaged orders to Seattle, WA
          ...
          #5 fastest merchants(1-5) to Denver, CO
          ...
          #5 slowest merchants(6-10) to Denver, CO
          ...
          #5 fastest merchants(11-15) to Seattle, WA
        ...
        context 'as a logged-in user' do
          ...
          it 'top 5 merchants who have fulfilled items the fastest to my state' do
            ...
            within("#top-five-fast-fulfilled-state") do
              expect(page).to have_content("#{@m[1].name}: 1 day 00 hours 00 minutes")
              expect(page).to have_content("#{@m[2].name}: 2 days 00 hours 00 minutes")
              expect(page).to have_content("#{@m[3].name}: 3 days 00 hours 00 minutes")
              expect(page).to have_content("#{@m[4].name}: 4 days 00 hours 00 minutes")
              ...
              expect(page).to_not have_content(@m[6].name) # slower fulfillment than top 5
              #merchants shipping to Seattle, WA fastest
              expect(page).to_not have_content(@m[11].name)
              expect(page).to_not have_content(@m[12].name)
              expect(page).to_not have_content(@m[13].name)
              expect(page).to_not have_content(@m[14].name)
        ...
        context 'as a merchant' do
          it 'visits merchants index page' do
            allow_any_instance_of(ApplicationController).to receive(:current_user).and_return(@m[1])

            visit merchants_path

            expect(page).to_not have_css('#top-five-fast-fulfilled-city')
            expect(page).to_not have_css('#top-five-fast-fulfilled-state')
          end
        end
      end
